...
def run_interview():
    ...
    print("[ACAD] 面試系統已連線，按 Enter 開始講話，講完再按 Enter 停止錄音並生成回應")
    print(" [SYMBOL]  Ctrl+C 隨時結束面試\n")

    try:
        while True:
            input("[MIC] 按 Enter 開麥，開始講話...")
            # 開始錄音（保險起見先清空 pending）
            manager.pending_student_texts = []
            manager.stt.start_recording()
            print("[READY] 麥克風已開啟，開始說話...（Enter 關麥）")
