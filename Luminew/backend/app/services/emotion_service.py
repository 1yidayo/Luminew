...
def _analyze_portfolio_sync(pdf_path: str) -> dict:
    ...
    try:
        ...
        特別提醒：我們不評判學生的經歷內容（例如：當社長很好、參加比賽很棒），而是分析他「如何呈現」這份檔案。務必分項回傳 JSON Array。" 
        特別提醒：strengths、weaknesses、suggestions 這三個欄位務必以「多個分項」形式回傳（JSON Array），請勿將多個觀點擠在同一個字串中。
        """
