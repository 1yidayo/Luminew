    ...
    try:
       ...
       # -----------------------------
        ...
        try:
            ...
            【你的任務】
            請使用第二人稱「你」直接對學生說話，給予一份**超級實用**的回饋。

            🚫 禁止事項（非常重要！）：
            - 不要只是重複說「你的自信指數是 XX%」這種廢話
            - 不要說「你展現了沈穩的一面」「情緒波動不大」這種沒營養的話
            - 不要泛泛地說「多練習就會進步」
            - 整個回覆中絕對不可使用「他/他的」或「該生」等第三人稱代名詞！
            ...
            {{
                ...
                "comment": (50-80 字的短評，使用「你/你的」直接稱呼學生，告訴學生你這次表現的亮點和需要改進的地方，要具體、有溫度，不要廢話，且絕對不可出現任何「他/他的」),
                "suggestion": (一句話的具體行動建議，用「你/你的」直接稱呼學生，例如「建議你下次回答問題前，先對面試官微笑並點頭，再開始說話」，絕對不可出現任何「他/他的」)
            }}
            """
            ...
            if c_score >= 50:
                ...
                fallback_comment = "你談�        prompt = f"""
        你是一位專業的高中升大學輔導專家，同時也是教育部「學習歷程檔案」的審閱委員。
        你正在審閱學生的學習歷程檔案，請給予專業的評價和具體的改進建議。
        ...
        {{
            ...
            "strengths": [
                "請使用『你/你的』，例如：你的優點1，不可使用『他/該生』",
                "請使用『你/你的』優點2，不可使用『他/該生』",
                "..."
            ],
            "weaknesses": [
                "請使用『你/你的』，例如：你需改進的缺點1，不可使用『他/該生』",
                "請使用『你/你的』需改進的缺點2，不可使用『他/該生』"
            ],
            "comment": (100-150字的整體評語，以『你/你的』直接稱呼學生，指出你這份學習歷程的亮點和可以加強的地方，要具體、有建設性，絕對不可使用『他/該生』),
            "suggestions": [
                "具體改進建議1（例如：建議你可以補充實作過程中遇到的困難和解決方法，使用『你/你的』，不可使用『他/該生』）",
                "具體改進建議2",
                "具體改進建議3"
            ]
        }}
